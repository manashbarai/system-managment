...
const LeadForSuperAdmin = (props) => {
    ...
    const [selectedCheckboxes, setSelectedCheckboxes] = useState(localStorage.getItem('givingLeads') ? JSON.parse(localStorage.getItem('givingLeads')) : []);
    const [checkboxes, setCheckboxes] = useState(myLeads);
    ...
    const openStatusOption = (e) => {
        setLeadId(e)
        setPpenStatusOptions(e)
    }
    ...
    const leadComments = (e) => {
        setLeadId(e)
        const name = myLeads.find(item => item._id === e)
    ...
    return (
        <div style={{height:"620px",overflow:"scroll"}} className=' '>
            ...
            <ul className='m-0 p-0  '>
                ...
                {props.myLeads && props.myLeads.map((e, i) => {
...
export default LeadForSuperAdmin
